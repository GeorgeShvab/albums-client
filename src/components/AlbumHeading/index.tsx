...
const AlbumHeading = ({
    ...
    const deleteAlbum = (): void => {
        ...
        if (mobile) {
            dispatch(
                showMobileMenu({ type: 'delete-album', data: { album: album } })
            )
            return
        }
        dispatch(showWindow({ type: 'delete-album', data: { album: album } }))
    }
    ...
    const changeName = (): void => {
        ...
        dispatch(
            showWindow({ type: 'change-album-name', data: { album: album } })
        )
    }
    ...
    const contextMenuElements = [
        ...
        {
            text: selectionMode.state ? 'Відмінити обирання' : 'Обрати фото',
            func: selectPhotos,
        },
        {
